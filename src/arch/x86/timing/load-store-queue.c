...
void x86_lsq_init()
{
	...
	X86_CORE_FOR_EACH X86_THREAD_FOR_EACH
	{
		...
		X86_THREAD.aq = linked_list_create(); /* Queue with commited stores */
...
void x86_lsq_done()
{
	struct linked_list_t *lq, *sq, *aq, *preq;
	...
	/* Commited store queue */
	X86_CORE_FOR_EACH X86_THREAD_FOR_EACH
	{
		aq = X86_THREAD.aq;
		linked_list_head(aq);
		while (linked_list_count(aq))
		{
			uop = linked_list_get(aq);
			uop->in_aq = 0;
			linked_list_remove(aq);
			x86_uop_free_if_not_queued(uop);
		}
		linked_list_free(aq);
	}
...
void x86_lsq_recover(int core, int thread)
{
	struct linked_list_t *lq = X86_THREAD.lq;
	struct linked_list_t *sq = X86_THREAD.sq;
	struct linked_list_t *aq = X86_THREAD.aq;
	struct x86_uop_t *uop;
	...
	/* Recover accessing queue */
	linked_list_head(aq);
	while (!linked_list_is_end(aq))
	{
		uop = linked_list_get(aq);
		if (uop->specmode)
		{
...
/* Insert uop into accessing queue */
void x86_aq_insert(struct x86_uop_t *uop)
{
	int core = uop->core;
	int thread = uop->thread;
	struct linked_list_t *aq = X86_THREAD.aq;
	...
	linked_list_out(aq);
	linked_list_insert(aq, uop);
	uop->in_aq = 1;
	X86_CORE.aq_writes++;
	X86_THREAD.aq_writes++;
	X86_CORE.aq_count++;
	X86_THREAD.aq_count++;
}


/* Remove an uop from the commited store queue */
void x86_aq_remove(struct x86_uop_t *uop)
{
	int core = uop->core;
	int thread = uop->thread;
	struct linked_list_t *aq = X86_THREAD.aq;
	...
	LINKED_LIST_FOR_EACH(aq)
	{
		struct x86_uop_t *queued_uop = linked_list_get(aq);

		assert(x86_uop_exists(queued_uop));
		assert(queued_uop->in_aq);
	...
	fatal("Uop not found: %s", __FUNCTION__);

done:
	assert(X86_CORE.aq_count && X86_THREAD.aq_count);
	X86_CORE.aq_count--;
	X86_THREAD.aq_count--;
}
