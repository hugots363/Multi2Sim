...
struct str_map_t heap_time_policy_map =
{
	3, {
		{ "FIFO", heap_time_policy_fifo },
		{ "LIFO", heap_time_policy_lifo },
		{ "Random", heap_time_policy_random }
	}
};
...
static int heap_less_than(struct heap_t *heap, int x, int y)
{
	...
	if (heap->time_policy == heap_time_policy_lifo)
		return heap->elem[x].time > heap->elem[y].time;

	/* random */
	assert(heap->time_policy == heap_time_policy_random);
	return random() % 2;
}
