...
char *network_switching_map[] = {"SAF", "VCT"};
enum network_switching_t network_switching;
...
struct net_t *net_create_from_config(struct config_t *config, char *name)
{
			...
			section = config_section_next(config))
	{
		...
		net->def_bandwidth = config_read_int(config, section,
				"DefaultBandwidth", 0);
		net->switching = config_read_enum(config, section, "Switching", network_switching_saf, network_switching_map, 2);
		...
		struct net_node_t *dst_node, int bus_src_buffer, int bus_dst_buffer)
{
	...
	{
		fatal("network \"%s\" : neither is a bus node.", net->name);
	}
		...
		void *receive_stack)
{
	...
	stack->msg = msg;
	stack->frag = list_get(msg->fragments, 0);

	msg->ret_event = receive_event;
	msg->ret_stack = receive_stack;
...
/* Fragments are marked as arrived with a flag when they finish their route */
void net_receive_frag(struct net_t *net, struct net_node_t *node, struct net_msg_frag_t *frag)
{
	struct net_buffer_t *buffer;
	...
	if (frag->node != node)
		panic("%s: fragment not at end node", __FUNCTION__);
	...
	/* Mark this fragment as arrived */
	frag->arrived = true;
	frag->parent->arrived_frags_count++;
}


/* When all fragments have arrived they are extracted from the buffer and freed */
void net_receive_msg(struct net_t *net, struct net_node_t *node, struct net_msg_t *msg)
{
	struct net_buffer_t *buffer;
	struct net_msg_frag_t *frag;

	for (int i = 0; i < list_count(msg->fragments); i++)
	{
		frag = list_get(msg->fragments, i);
		buffer = frag->buffer;
