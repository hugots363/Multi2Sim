...
static char *x86_checkpoints_dir = ".";
...
static char *m2s_help =
		...
		"================================================================================\n"
		"\n"
		"  --x86-checkpoints-dir <directory>\n"
		"      Set a specific directory where all the checkpoints created when a signal is\n"
		"      received will be stored. By default they will be stored in '.'.\n"
		"\n"
		"  --x86-config <file>\n"
...
static void m2s_read_command_line(int *argc_ptr, char **argv)
{
	...
	for (argi = 1; argi < argc; argi++)
	{
		...
		{
			m2s_need_argument(argc, argv, argi);
			srand(atoll(argv[++argi]));
			continue;
		}

		/* Determines which event is executed first when multiple are scheduled to the same cycle */
		if (!strcmp(argv[argi], "--heap-time-policy"))
		{
			m2s_need_argument(argc, argv, argi);
			event_heap_time_policy = str_map_string_case_err_msg(&heap_time_policy_map, argv[++argi], "invalid value for --heap-time-policy.");
			continue;
		}
		...
		/* Directory for storing checkpoints created when a SIGUSR2 or SIGTERM signal is received */
		if (!strcmp(argv[argi], "--x86-checkpoints-dir"))
		{
			m2s_need_argument(argc, argv, argi);
			x86_checkpoints_dir = argv[++argi];
			filesystem_dir_create("", x86_checkpoints_dir);
			continue;
		}
...
static void m2s_signal_process(void)
{
	...
	case SIGUSR1:
	{
		...
		/* Summary file name */
		snprintf(file_name, sizeof file_name, "m2s-summary-%s-%lld", m2s_sim_id, time_in_dsec);
		warning("SIGUSR1 received, dumping summary in '%s'", file_name);
		...
		/* Detailed report file name */
		snprintf(file_name, sizeof file_name, "m2s-detailed-%s-%lld", m2s_sim_id, time_in_dsec);
		warning("and detailed report in '%s'\n", file_name);
	...
	case SIGUSR2:
	{
		...
		snprintf(buf, MAX_PATH_SIZE, "%s/chk_%d", x86_checkpoints_dir, chk_id);
		warning("SIGUSR2 signal received, checkpointing in '%s'\n", buf);
		x86_checkpoint_save(buf);
		chk_id++;
		break;
	}
	...
	case SIGTERM:
	{
		char buf[MAX_PATH_SIZE];
		snprintf(buf, MAX_PATH_SIZE, "%s/chk_%d", x86_checkpoints_dir, chk_id);
		warning("SIGTERM signal received, checkpointing in '%s' and finishing the simulation\n", buf);
		x86_checkpoint_save(buf);
		chk_id++;
...
static void m2s_loop(void)
{
	...
	signal(SIGTERM, &m2s_signal_handler);
